$(document).ready(function() {
    ...
    //var url = "http://localhost/matriz/auth.php?callback=?";
    var url = "https://sanfranciscodekkerlab.com/matriz/auth.php?callback=?";
   ...
   //Funcion de registro
    $("#signup").click(function() {
        ...
        var direccion = calle + "," + numero + "," + colonia + "," + estado + "," + ciudad + "," + cp;
        ...
        var dataString = "fullname=" + fullname + "&email=" + email + "&password=" + password + "&taller=" + taller + "&telefono=" + telefono + "&celular=" + celular + "&direccion=" + direccion + "&latitud=" + latitud + "&longitud=" + longitud + "&signup=";
        if ($.trim(fullname).length > 0 & $.trim(email).length > 0 & $.trim(password).length > 0 & $.trim(taller).length > 0 & $.trim(celular).length > 0 & $.trim(calle).length > 0 & $.trim(numero).length > 0 & $.trim(colonia).length > 0 & $.trim(estado).length > 0 & $.trim(ciudad).length > 0 & $.trim(cp).length > 0  & $.trim(direccion).length > 0 & $.trim(latitud).length > 0 & $.trim(longitud).length > 0) {
            $.ajax({
                type: "POST",
                url: url,
                data: dataString,
                ...
                success: function(data) {
                    if (data == "success") {
                        ...
                        swal("Gracias por registrarse", "puede iniciar sesión ahora", "success");
                        window.location.href = "login.html";
                    } else if (data = "exist") {
                        swal("Hey! Su cuenta ya se encuentra registrada!", "puede iniciar sesión ahora", "info");
                    } else if (data = "failed") {
    ...
    $("#guardarDatosFacturacion").click(function() {
        ...
        var dataString ="idCliente=" + idCliente + "&rfc=" + rfc + "&razonSocial=" + razonSocial + "&dirFiscal=" + dirFiscal + "&cp=" + cp + "&correoEnvio=" + correoEnvio + "&cfdi=" + cfdi + "&datosFacturacion=";
        if ($.trim(idCliente).length > 0 & $.trim(rfc).length > 0 & $.trim(razonSocial).length > 0 & $.trim(dirFiscal).length > 0 & $.trim(cp).length > 0 & $.trim(correoEnvio).length > 0 & $.trim(cfdi).length > 0 ) {
            $.ajax({
                type: "POST",
                url: url,
                data: dataString,
    ...
    $("#editarDatosFacturacion").on("click", "#guardarCambiosEdit", function() {
        ...
        var dataString = "idCliente=" + idCliente + "&editarRfc=" + editarRfc + "&editarRazonSocial=" + editarRazonSocial + "&editarDirFiscal=" + editarDirFiscal + "&editarCp=" + editarCp + "&editarCorreoEnvio=" + editarCorreoEnvio + "&editarCfdi=" + editarCfdi + "&actualizarDatosFacturacion=";
        if ($.trim(idCliente).length > 0 & $.trim(editarRfc).length > 0 & $.trim(editarRazonSocial).length > 0 & $.trim(editarDirFiscal).length > 0 & $.trim(editarCp).length > 0 & $.trim(editarCorreoEnvio).length > 0 & $.trim(editarCfdi).length > 0) {
            $.ajax({
                type: "POST",
                url: url,
                data: dataString,
                ...
                success: function(data) {
                    if (data = "success") {
                        ...
                        console.log(data);
                        var dataString = "idCliente=" + idCliente + "&listarDatosFacturacion=";
    ...
    $("#formContact").click(function() {
        ...
        var dataString = "nameApellidos=" + nameApellidos + "&emailContacto=" + emailContacto + "&comentariosContacto=" + comentariosContacto + "&formContact=";
        if ($.trim(nameApellidos).length > 0 & $.trim(emailContacto).length > 0 & $.trim(comentariosContacto).length > 0) {
            $.ajax({
                type: "POST",
                url: url,
                data: dataString,
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data == "invalid") {
                        ...
                        swal("Algo salio Mal", "Su cuenta no se encuentra registrada", "info");
                    } else if (data = "success") {
                        swal("", "La contraseña ha sido enviada a la cuenta de correo, porfavor revise", "success");
    ...
    $("#formProcesarSolicitud").click(function() {
        ...
        var dataString = "observacionesSolicitud=" + observaciones + "&observacionesProductos=" + observacionesProductos +"&listaProductos=" + listaProductos + "&sucursalElegida=" + sucursalElegida + "&nombreCliente=" + nombreCliente + "&idCliente=" + idCliente + "&formaPago=" + formaPago + "&solicitudProceso=";
        ...
        if (sucursalElegida === undefined) {
            ...
            if(listaProductos == "[]" && ob1 == true && observacionesProductos == "" && ob2 == false || observacionesProductos != ""){
             swal({
            ...
            .then((willDelete) => {
              if (willDelete) {
                ...
                if ($.trim(sucursalElegida).length > 0 & $.trim(nombreCliente).length > 0 & $.trim(idCliente).length > 0) {
                $.ajax({
                    type: "POST",
                    url: url,
                    data: dataString,
                ...
                success: function(data) {
                    if (data != "failed") {
                        ...
                        for (var clave in json) {
                            ...
                            localStorage.motoEnCaminoFecha = JSON.parse(json).motoEnCaminoFecha;
                            localStorage.enProceso = JSON.parse(json).enProceso;
                            localStorage.enProcesoFecha = JSON.parse(json).enProcesoFecha;
                            localStorage.motoEnCaminoRegreso = JSON.parse(json).motoEnCaminoRegreso;
                    ...
                    if (data != "failed") {
                        ...
                        for (x = 0; x < types.length; x++) {
                            ...
                            if (existe == 1) {
                                ...
                                var dataString = "idSolicitud=" + idSolicitud + "&actualizarRequiereFactura=";
                                if ($.trim(idSolicitud).length > 0) {
                                    $.ajax({
                                        type: "POST",
                                        url: url,
    ...
    $(".tablaLista").on("click", ".btnrequiereFacturaRed", function() {
        ...
        var dataString = "idSolicitudRed=" + idSolicitudRed + "&actualizarRequiereFacturaRed=";
                ...
                success: function(data) {
                    if (data != "failed") {
                        ...
                        for (var clave in json) {
                            ...
                            var local = localStorage.rutaSolicitud = JSON.parse(json).rutaSolicitud;
   
                        }
                        var json = localStorage.rutaSolicitud;
                        window.open(`https://sanfranciscodekkerlab.com/matriz/`+json+``, '_system');
    ...
    $("#tipo").click(function() {
        ...
        if (empresa == "San Manuel" || empresa == "Reforma" || empresa == "Santiago") {
        ...
        var dataString = "idDesglose=" + idDesglose +  "&marca=" + marca + "&listarProductosDesglose=";
                ...
                success: function(data) {
                    if (data != "failed") {
                        ...
                        for (x = 0; x < typesDesglose.length; x++) {
                         ...
                         var div3 = `<option value="`+typesDesglose[x].id+`">`+typesDesglose[x].descripcion+`</option>`;
     ...
     $("#desglose").click(function() {
        ...
        var dataString = "idSubcategoriaDesglose=" + idSubcategoriaDesglose + "&listarProductosSubDesglose=";
                ...
                success: function(data) {
                    if (data != "failed") {
                        ...
                        for (x = 0; x < typesDesglose.length; x++) {
                         ...
                         var div4 = `<option value="`+typesDesglose[x].id+`">`+typesDesglose[x].codigo+` | `+typesDesglose[x].descripcion+`</option>`;
    ...
    $("#subdesglose").click(function() {
        ...
        var dataString = "idProductoDesglose=" + idProductoDesglose + "&mostrarProductoFinal=";
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data != "failed") {
                        ...
                        var miObjeto = [{"precio":precioCubeta,"valor":"CUBETA"},{"precio":precioGalon,"valor":"GALÓN"},{"precio":precioLitro,"valor":"LITRO"},{"precio":precioMedio,"valor":"500 ML"},{"precio":precioCuarto,"valor":"250 ML"},{"precio":precioOctavo,"valor":"125ML"}];
                        ...
                        for (var i = 0; i < preciosP.length; i++) {
                            ...
                            var div5 = `<option value="`+preciosP[i].precio+`">`+preciosP[i].valor+`</option>`;
                            $("#unidad").append(div5);
                        }
    ...
    $("#contenedor2").on("click", ".btnAgregarP", function() {
        ...
        if (cantidad != 0) {
            function Producto(idCliente, idProducto, codigoProducto, precioProducto,unidadMedida, cantidad) {
            ...
            nuevoProducto = new Producto(idCliente, idProducto, codigoProducto,  precioProducto, unidadMedida, cantidad);
            agregar();
     ...
     $("#contenedors").on("click", ".btnAgregarProd", function() {
        ...
        if (cantidad != 0 ) {
            function Producto(idCliente, idProducto, descripcion, codigoProducto, precioProducto, unidadMedida, foto, precioDescuento, descuento, cantidad) {
                this.idCliente = idCliente;
                this.idProducto = idProducto;
            ...
            if (arregloProductos == "[]") {
                 ...
                 nuevoProducto = new Producto(idCliente, idProducto, descripcion, codigoProducto,  precioProducto, unidadMedida, foto, precioDescuento, descuento, cantidad);
                 agregar();
                 swal("Exito!", "Producto agregado al Carrito", "success");
                            ...
                            for (x = 0; x < types.length; x++) {
                                ...
                                if (types[x].favorito == 1) {
                                    ...
                                    if (descuento != 0) {
                                        ...
                                        var div = `<div class="col-lg-12 col-md-12 col-sm-12" id="contBusqueda">
                                            <div id="svgOff" >
                                                ...
                                                <center>
                                                    <table id="tableBotones" >
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td colspan="3" style="border-top:2px;">
                                                                <button style = "margin-top:0%;margin-right: 2px;" type="button" id="btnCorazon" class="btnCorazon" idProducto ="`+types[x].idMvendido+`" descripcion="`+types[x].descripcion+`" codigoProducto ="`+types[x].codigoProducto+`" precioProducto ="`+types[x].precio+`" unidadMedida ="`+types[x].unidadMedida+`" foto="`+types[x].foto+`"><i class="fas fa-heart"></i></button>
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </center>
    ...
    $("#tableResultadosBusqueda").on("click", ".btnCorazon", function() {
        ...
        var dataString = "idProducto=" + idProducto + "&idCliente=" + idCliente + "&codigoProducto=" + codigoProducto + "&precioElegido=" + precioElegido + "&unidadElegida=" + unidadElegida + "&addFavoritos=";
        if ($.trim(idProducto).length > 0 & $.trim(idCliente).length > 0) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    swal("Upss...", "El producto ya se encuentra agregado a mis favoritos.", "info");
    ...
    $("#tableResultadosBusqueda").on("click", ".btnCorazonRed", function() {
        ...
        var dataString = "idProductoF=" + idProductoF + "&idCliente=" + idCliente + "&deleteFavoritosRed=";
                                                ...
                                                <table id="tableProductos">
                                                  <tr>
                                                    ...
                                                    <div id="svgOff" >
                                                      ...
                                                      <center>
                                                          <img src="`+types[x].foto+`" alt="" style="width:30%; height:10%;">
                                                      </center>
                                                      <p style="margin-left:10%;width:90%;height:25px;">
                                                        <span type="text" id="nameProductoS">`+types[x].descripcion+`</span>
                                                      </p>
                                                      ...
                                                      <button type="button" id="btnCorazonRed" class="btnCorazonRed" idProductoF ="`+types[x].idMvendido+`"><i class="fas fa-heart"></i></button>
                                                       
                                                       <input type="hidden" name="cantidadProd[]" valor="1" maxlength="2" max="50" size="1" id="cantidadProd" style="" class="" placeholder="" value="1"/>
                                                       <button type="button" class="btn btn-success btnAgregaraCarro" id="btnAgregaraCarro" idProducto ="`+types[x].idMvendido+`" descripcion="`+types[x].descripcion+`" codigoProducto ="`+types[x].codigoProducto+`" precioProducto ="`+types[x].precio+`" unidadMedida ="`+types[x].unidadMedida+`" foto="`+types[x].foto+`" precioDescuento="`+precioF.toFixed(2)+`" descuento="`+descuento+`">Agregar <i class="fas fa-shopping-cart"></i></button>
                                                      </td>
                                                 </tr>
                                                </table>`;
                                            }else{
                                                ...
                                                <table id="tableProductos">
                                                  <tr>
                                                    <td>
                                                      ...
                                                      <button type="button" id="btnCorazon" class="btnCorazon" idProducto ="`+types[x].idMvendido+`" descripcion="`+types[x].descripcion+`" codigoProducto ="`+types[x].codigoProducto+`" precioProducto ="`+types[x].precio+`" unidadMedida ="`+types[x].unidadMedida+`" foto="`+types[x].foto+`"><i class="fas fa-heart"></i></button>
                                                      <input type="hidden" name="cantidadProd[]" valor="1" maxlength="2" max="50" size="1" id="cantidadProd" style="" class="" placeholder="" value="1"/>
                                                         <button type="button" class="btn btn-success btnAgregaraCarro" id="btnAgregaraCarro" idProducto ="`+types[x].idMvendido+`" descripcion="`+types[x].descripcion+`" codigoProducto ="`+types[x].codigoProducto+`" precioProducto ="`+types[x].precio+`" unidadMedida ="`+types[x].unidadMedida+`" foto="`+types[x].foto+`" precioDescuento="`+precioF.toFixed(2)+`" descuento="`+descuento+`">Agregar <i class="fas fa-shopping-cart"></i></button>                           
                                                      </td>
                                                 </tr>
                                            ...
                                            <table id="tableProductosBus">
                                              <tr>
                                                <td>
                                                  ...
                                                  <button type="button" class="btn btn-success btnAgregaraCarroBus" id="btnAgregaraCarroBus" idProducto ="`+types[x].idMvendido+`" descripcion="`+types[x].descripcion+`" codigoProducto ="`+types[x].codigoProducto+`" precioProducto ="`+types[x].precio+`" unidadMedida ="`+types[x].unidadMedida+`" foto="`+types[x].foto+`" precioDescuento="`+precioF.toFixed(2)+`" descuento="`+descuento+`">Agregar <i class="fas fa-shopping-cart"></i></button>
                                                </td>
                                              </tr>
                                            </table>`;
                                          }else{
    ...
    */
    ...
    $("#contenedors").on("click", ".favoritosEliminar", function() {
        ...
        var idProductoF = $(this).attr("idProductoF");
        var dataString = "idProducto=" + idProducto + "&idProductoF=" + idProductoF + "&idCliente=" + idCliente + "&deleteFavoritos=";
    ...
    var imageHash = "http://sanfranciscodekkerlab.com/images/plantilla-correo/LOGOSDFblanco.png";
    ...
    if (localStorage.concluido == 1) {
        $("#saludoConcluido").html("Solicitud Concluida.<br>Gracias por realizar su solicitud con nosotros");
    } else {
